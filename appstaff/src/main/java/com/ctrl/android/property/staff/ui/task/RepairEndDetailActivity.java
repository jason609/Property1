 ...
 */
public class RepairEndDetailActivity extends AppToolBarActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_repair_end_detail);
    ...
    public void onRequestSuccess(int requestCode) {
        ...
        if(requestCode==3){
            MessageUtils.showShortToast(RepairEndDetailActivity.this,"下达任务成功");
            finish();
        }
        ...
        if(requestCode==1){
           ...
           tv_excute_name.setText("执行人："+rdao.getRepair().getStaffName());
           if(rdao.getRepair().getAssignTime()!=null){
               tv_appiont_time.setText("派工时间："+D.getDateStrFromStamp("yyyy-MM-dd HH:mm",rdao.getRepair().getAssignTime()));
           }
        }
    }
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()){
            case R.id.iv_excute_add:
                intent=new Intent(RepairEndDetailActivity.this, ContactActivity.class);
                startActivityForResult(intent, 200);
                AnimUtil.intentSlidIn(RepairEndDetailActivity.this);
                break;
            case R.id.tv_done:
                if(!TextUtils.isEmpty(tv_excute_name.getText().toString())){
                    rdao.requestAssignComplaint(id,rdao.getRepair().getId());
                }
                break;
        }
