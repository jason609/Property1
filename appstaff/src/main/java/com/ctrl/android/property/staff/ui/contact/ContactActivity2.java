...
import com.ctrl.android.property.staff.ui.adapter.ContractAdapter2;
 ...
 */
public class ContactActivity2 extends AppToolBarActivity implements View.OnClickListener{
    ...
    private ContractAdapter2 contactAdapter;
...
//
//            contactAdapter = new ContractAdapter2(this);
...
//            listContractGroup.get(index).setListContactor(contactDao.getListContactor());
            ...
            listContactor = contactDao.getListContactor();
            contactAdapter = new ContractAdapter2(this);
            contactAdapter.setList(listContactor);
            listView.setAdapter(contactAdapter);
            //contactAdapter.notifyDataSetChanged();
            listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    AppHolder.getInstance().setContactor(listContactor.get(position));
                   /* Intent intent=new Intent();
                    intent.putExtra("name",listContactor.get(position).getName());
                    intent.putExtra("id",listContactor.get(position).getId());
